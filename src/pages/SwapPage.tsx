...
export default function SwapPage({ wallet }: SwapPageProps) {
  ...
  const currentPool = useMemo(() => {
    if (!inputToken || !outputToken || pools.length === 0) return null;
    return pools.find(
      (p) =>
        (p.token0.denom === inputToken.denom && p.token1.denom === outputToken.denom) ||
        (p.token0.denom === outputToken.denom && p.token1.denom === inputToken.denom)
    ) || null;
  }, [inputToken, outputToken, pools]);
  ...
  useEffect(() => {
    async function load() {
      setLoading(true);
      try {
        const [tokenList, poolList] = await Promise.all([getTokens(), getPools()]);
        setTokens(tokenList);
        setPools(poolList);
        // Default to SLTN
  ...
  const handleSwap = async () => {
    if (!wallet.connected || !inputToken || !outputToken || !quote || !currentPool) {
      if (!currentPool) {
        showToast('No liquidity pool found for this pair', 'error');
        return;
      }
      return;
    }
    ...
    setSwapping(true);
    try {
      const result = await executeSwapSigned({
        pairId: currentPool.id,
      ...
      showToast(`Swapped ${inputAmount} ${inputToken.symbol} for ${formatAmount(result.amount_out, outputToken.decimals)} ${outputToken.symbol}`, 'success');
      setInputAmount('');
      setQuote(null);
    } catch (error) {
      console.error('Swap failed:', error);
      showToast(error instanceof Error ? error.message : 'Swap failed. Please try again.', 'error');
    } finally {
      setSwapping(false);
    }
  };
