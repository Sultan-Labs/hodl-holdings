...
export default function PoolsPage({ wallet }: PoolsPageProps) {
  ...
  const handleAddLiquidity = async () => {
    ...
    try {
      const result = await addLiquiditySigned({
        pairId: selectedPool.id,
      ...
      showToast(`Added liquidity: ${result.lp_tokens} LP tokens`, 'success');
      setShowAddModal(false);
      setAmount0('');
      setAmount1('');
    } catch (error) {
      console.error('Failed to add liquidity:', error);
      showToast(error instanceof Error ? error.message : 'Failed to add liquidity', 'error');
    } finally {
  ...
  const handleRemoveLiquidity = async () => {
    ...
    try {
      ...
      const result = await removeLiquiditySigned({
        poolId: selectedPosition.poolId,
        lpTokens: lpToRemove,
        minAmount0: '0',
        minAmount1: '0',
      });
      showToast(`Removed liquidity: received ${formatAmount(result.amount_a)} + ${formatAmount(result.amount_b)}`, 'success');
      setShowRemoveModal(false);
    } catch (error) {
      console.error('Failed to remove liquidity:', error);
      showToast(error instanceof Error ? error.message : 'Failed to remove liquidity', 'error');
    } finally {
