...
export default function LaunchpadPage({ wallet }: LaunchpadPageProps) {
  ...
  const handleCreateToken = async () => {
    ...
    setCreating(true);
    try {
      const result = await createTokenSigned({
        name: tokenName,
      ...
      setCreatedToken({ denom: result.denom, txHash: result.denom });
      ...
      showToast(error instanceof Error ? error.message : 'Failed to create token. Please try again.', 'error');
    } finally {
      setCreating(false);
    }
  };
