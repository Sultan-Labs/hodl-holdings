...
function App() {
  ...
  useEffect(() => {
    const unsubConnect = onWalletEvent('connect', () => {
    ...
    const unsubAccountChange = onWalletEvent('accountChange', () => {
      getWalletState().then(setWallet);
    });
            ...
            {wallet.connected ? (
              <div className="flex items-center gap-2">
                <div className="glass-card px-3 py-2 text-sm">
                  <span className="text-white/70">{parseFloat(wallet.balance) / 1e9} SLTN</span>
                </div>
                <button
